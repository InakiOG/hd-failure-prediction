...
import pySMART # pip install pySMART 
import csv # pip install csv
from datetime import date

# Issue found: https://www.smartmontools.org/ticket/1223

def print_smart_data(drives):

  for drive in drives:
    smart_data = drive.all_attributes()

    print(f"SMART data for drive {drive.name}:")
    print(smart_data)

def save_smart_data_as_csv(drives, filename):
    ...
    with open(filename, 'w', newline='') as csvfile:
      ...
      for drive in drives:
        row = [
          ...
          # drive.pod_id,
          # drive.pod_slot_num,
          # drive.is_legacy_format
        ]
        smart_data = drive.all_attributes()
        if smart_data:
          row.extend(smart_data)
        writer.writerow(row)
    print("SMART data saved as smart_data.csv")
  except Exception as e:
    print(f"Error saving SMART data as CSV: {e}")

drives = pySMART.DeviceList()

print(f"drives: {drives}")

if not drives.devices: raise Exception("No devices found.")

print_smart_data(drives)
save_smart_data_as_csv(drives, 'smart_data.csv')